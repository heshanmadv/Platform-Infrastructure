...
3. **Run the playbook:**
   ...
   - `roles/k3s-server` on the `k3s_server` host — installs k3s via the
     ...
     at `kubeconfig/k3s.yaml` (gitignored — never commit it) and rewrites
     its server URL from `127.0.0.1` to the Pi's real address.
...
- This is deliberately **single-node**: one Pi is both the k3s server and
  ...
  target it at.
- Terraform (Phase 1 onward) consumes `kubeconfig/k3s.yaml` produced here.
  Ansible must run first.
